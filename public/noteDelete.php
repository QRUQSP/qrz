...
//
function qruqsp_qrz_noteDelete(&$ciniki) {
    //
    ...
    $rc = qruqsp_qrz_checkAccess($ciniki, $args['tnid'], 'qruqsp.qrz.noteDelete');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    $strsql = "SELECT id, uuid "
        ...
        . "";
    $rc = ciniki_core_dbHashQuery($ciniki, $strsql, 'qruqsp.qrz', 'note');
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    if( !isset($rc['note']) ) {
    ...
    $rc = ciniki_core_objectCheckUsed($ciniki, $args['tnid'], 'qruqsp.qrz.note', $args['note_id']);
    if( $rc['stat'] != 'ok' ) {
    ...
    // Remove the note
    //
    $rc = ciniki_core_objectDelete($ciniki, $args['tnid'], 'qruqsp.qrz.note',
