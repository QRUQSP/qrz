...
//
function qruqsp_qrz_checkAccess(&$q, $station_id, $method) {
    //
    // Check if the station is active and the module is enabled
    //
    qruqsp_core_loadMethod($q, 'qruqsp', 'core', 'private', 'checkModuleAccess');
    $rc = qruqsp_core_checkModuleAccess($q, $station_id, array('package'=>'qruqsp', 'module'=>'qrz'));
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( $rc['stat'] != 'ok' ) {
        return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.qrz.1', 'msg'=>'Access denied.'));
    }
    //
    ...
    // By default fail
    //
    return array('stat'=>'fail', 'err'=>array('code'=>'qruqsp.qrz.2', 'msg'=>'Access denied'));
}
?>
